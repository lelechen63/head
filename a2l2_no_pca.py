...
from dataset import  Grid_audio_lmark_single as Grid_audio_lmark

# from ATVG import AT_single2, AT_single2_no_pca 
...
#         self.opt_g = torch.optim.SGD(self.generator.parameters(), lr=config.lr)  
        if config.dataset_name == 'vox':
            ...
            if not config.seperate:
                ...
                self.dataset = Grid_audio_lmark('/data/lchen63/grid/zip/txt/', 'train')
            else:
                self.dataset = Grid_audio_lmark('/data/lchen63/grid/zip/txt/', 'train')
    ...
    def fit(self):
        ...
        if config.dataset_name == 'vox':
            ...
            face_mean =torch.FloatTensor( np.load('./basics/mean_grid_roni_norm.npy')).cuda()        
            lip_mean =torch.FloatTensor( np.load('./basics/mean_grid_lip_norm.npy')).cuda()
            lip_pca = torch.FloatTensor( np.load('./basics/U_grid_lip_norm.npy')[:,:6]).cuda()
            face_pca = torch.FloatTensor( np.load('./basics/U_grid_roni_norm.npy')[:,:6]).cuda()
            lip_std = torch.FloatTensor( np.load('./basics/std_grid_lip_norm.npy')).cuda()
            face_std = torch.FloatTensor( np.load('./basics/std_grid_roni_norm.npy')).cuda()
            
            pca_mean  =torch.FloatTensor( np.load('./basics/mean_grid_norm.npy')).cuda()
            pca_std = torch.FloatTensor( np.load('./basics/std_grid_norm.npy')).cuda()
            pca_pca = torch.FloatTensor( np.load('./basics/U_grid_norm.npy')[:,:6]).cuda()
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            for step, data in enumerate(self.data_loader):
                ...
                if config.seperate:
                    ...
                    if config.openrate_loss:
                        for k in range(3):
                            real_open.append(lip_region_pca[:,open_pair[k][0]*3 : open_pair[k][0]*3 + 2] - lip_region_pca[:,open_pair[k][1]*3 : open_pair[k][1]*3 + 2])
                            fake_open.append(fake_lip[:,open_pair[k][0]*3: open_pair[k][0]*3 + 2] - fake_lip[:,open_pair[k][1] * 3 : open_pair[k][1]*3+ 2])
    ...
    #                 logger.scalar_summary('loss_lower', loss_lower,epoch * num_steps_per_epoch + step+1)
                    if config.openrate_loss:
                        logger.scalar_summary('loss_open', loss_open,epoch * num_steps_per_epoch + step+1)
...
#                            .format(config.model_dir))
                    ...
                    target_pca = (target_lmark - pca_mean.expand_as(target_lmark))/pca_std
                
                    if config.pca:
                        target_pca = torch.mm(target_pca,  pca_pca)

                        
                    ex_pca = (ex_lmark - pca_mean.expand_as(ex_lmark))/pca_std
                ...
                fake_lmark = fake_lmark  * pca_std + pca_mean.expand_as(fake_lmark)
                ...
                real_lmark = real_lmark  * pca_std + pca_mean.expand_as(real_lmark)
...
def parse_args():
    ...
    parser.add_argument('--openrate_loss', type=bool, default= False)
...
#         from ATVG import AT_single2_no_pca as atnet
        ...
        config.model_name += '_no_pca'
    
        if config.openrate_loss:
            config.model_name = config.model_name + '_openrate_loss3'
    if config.dataset_name == 'grid':
        config.model_name += '_grid'
    
    
    import a2l2_no_pca as trainer
