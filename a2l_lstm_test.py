...
def test( config):            
        ...
        dataset = Voxceleb_audio_lmark('/data2/lchen63/voxceleb/txt/', config.is_train)
...
#                 lip_region_pca = lip_region_pca.view(lip_region_pca.shape[0] *lip_region_pca.shape[1], -1)
            ...
            lip_region_pca =  lip_region_pca.view(lip_region_pca.size(0) * time_length  , 6) 
            
            other_region_pca =  other_region_pca.view(other_region_pca.size(0) * time_length  , 6) 
            ...
            for gg in range(int(config.batch_size)):
                ...
                print ('{}/{}: mse: {}:openrate: {}'.format(step, gg, mse, openrate))
...
def parse_args():
    ...
    parser.add_argument('--openrate', type=bool, default=False, help='True or False')
    parser.add_argument('--visualize', type=bool, default=False, help='True or False')
...
if __name__ == "__main__":
    ...
    if config.pca:
        if config.map_length == '7':
            from dataset import  Voxceleb_audio_lmark_lstm  as Voxceleb_audio_lmark
            from ATVG import AT_lstm as atnet
            config.model_name += '_pca'      
            config.model_dir = "./model/at_lstm/anet_lstm.pth"
        ...
        if config.openrate:
            config.model_dir = "./model/at3_no_pca/anet2_single.pth"
