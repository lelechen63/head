...
import random
from dataset import  Voxceleb_face_region_single
from ATVG import FaceNet_single, FaceNet_Discriminator
...
#             self.l1_loss_fn =  nn.L1Loss().cuda()
        ...
        if config.gan:
            ...
            lr=config.lr, betas=(config.beta1, config.beta2))
        self.dataset = Voxceleb_face_region_single( config.dataset_dir, config.is_train)
    ...
    def test(self):
        ...
        cc = 0
        t0 = time.time()
        self.generator.load_state_dict( torch.load('/home/lchen63/head/model/face_encoder_single/vg_net.pth'))
        self.generator.eval()
        for epoch in range(self.start_epoch, config.max_epochs):            
            for step, (data) in enumerate(self.data_loader):
                ...
                lmark_base = Variable(lmark_base.view(in_lmark.shape[0], -1).cuda())
                ...
                ex_store = lip_base.data.contiguous().view(config.batch_size ,3,256,256)
                torchvision.utils.save_image(ex_store,
                    "{}/img_ex_{}.png".format(config.sample_dir,cc),normalize=True)
                cc += 1
...
if __name__ == "__main__":
    ...
    import face_encoder3 as trainer
    if config.is_train == 'test':
        config.sample_dir = './test_result/'
