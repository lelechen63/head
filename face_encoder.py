...
from ATVG import FaceNet, FaceNet_Discriminator,  Vgg16
...
class Trainer():
    def __init__(self, config):            
        ...
        if config.perceptual:
            self.vgg = Vgg16(requires_grad=False).cuda(device=config.cuda1)
            
#             self.vgg = nn.DataParallel(self.vgg, device_ids=device_ids).cuda()
# #########single GPU#######################
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            ...
            for step, (data) in enumerate(self.data_loader):
                ...
                lip_base = lip_base.view(config.batch_size, 64*3, 256,256)
                ...
                if config.gan:
                    ...
                    if config.perceptual:
                        real_features = self.vgg(gt_img)
                        fake_features = self.vgg(fake_img)
                        perceptual_loss = self.mse_loss_fn(real_features.relu2_2, fake_features.relu2_2)
                    ...
                    if config.perceptual:
                        logger.scalar_summary('perceptual_loss', perceptual_loss,epoch * num_steps_per_epoch + step+1)
...
if __name__ == "__main__":
    ...
    import face_encoder as trainer
    if config.perceptual:
        config.model_name = config.model_name+ '_percep'
