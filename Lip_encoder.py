...
import random
from dataset import  Voxceleb_lip_region as Voxceleb_lip_region
from ATVG import LipNet, LipNet_Discriminator
...
class Trainer():
    def __init__(self, config):            
        self.generator = LipNet( )
        
        self.l1_loss_fn =  nn.L1Loss()
        self.mse_loss_fn = nn.MSELoss()
        self.config = config
        if self.config.gan:
            self.discriminator = LipNet_Discriminator()
...
#             self.l1_loss_fn =  nn.L1Loss().cuda()
        ...
        if config.gan:
            ...
            lr=config.lr, betas=(config.beta1, config.beta2))
        self.dataset = Voxceleb_lip_region( config.dataset_dir, 'train')
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            ...
            for step, (data) in enumerate(self.data_loader):
                ...
                if (step) % (int(num_steps_per_epoch  / 32 )) == 0: #and step != 0:
                    ...
                    fake_store = fake_img.data.contiguous().view(config.batch_size,3,64,64)
                    torchvision.utils.save_image(fake_store,
                        "{}img_fake_{}.png".format(config.sample_dir,cc),normalize=True)
                    
                    real_store = gt_img.data.contiguous().view(config.batch_size ,3,64,64)
