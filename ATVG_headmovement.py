...
from dataset import  Voxceleb_head_movements_derivative2 as Voxceleb_head_movements_derivative
from ATVG import AT_net2 as AT_net
    ...
    def fit(self):
        ...
        for epoch in range(self.start_epoch, config.max_epochs):
            for step, (data) in enumerate(self.data_loader):
                t1 = time.time()
                in_landmark = data['in_lmark'].cuda()
                out_landmark = data['out_lmark'].cuda()
#                 meanl = data['mean']
                in_derivative = torch.zeros(in_landmark.size(), dtype = torch.float32).cuda()
                in_derivative[:,:-1] = (in_landmark[:,1:] - in_landmark[:,:-1])* 2.0/torch.FloatTensor([16,16,32]).cuda()
                out_derivative = Variable(torch.zeros(out_landmark.size(), dtype = torch.float32).cuda())
                out_derivative[:,:-1] = (out_landmark[:,1:] - out_landmark[:,:-1])*  2.0/torch.FloatTensor([16,16,32]).cuda()  

                in_derivative = in_derivative.view(in_derivative.shape[0],in_derivative.shape[1],-1)
                out_derivative = out_derivative.view(out_derivative.shape[0],out_derivative.shape[1],-1)
                
                in_derivative = Variable(in_derivative)
                out_derivative = Variable(out_derivative)
                
                
        
                fake_derivative= self.generator( in_derivative)

                loss =  self.mse_loss_fn(fake_derivative , out_derivative) 
                loss.backward() 
                self.opt_g.step()
                ...
                if (step) % (int(num_steps_per_epoch  / 1 )) == 0: #and step != 0:
                    print (fake_derivative[0,3:5])
                    print (out_derivative[0,3:5])
                    with open(log_name, "a") as log_file:
                        
                        log_file.write('fake%s\n' % str(fake_derivative[0,3:5]))
                        log_file.write('gt%s\n' % str(out_derivative[0,3:5]))
...
if __name__ == "__main__":
    ...
    import ATVG_headmovement as trainer
