...
class Lmark2RgbGenerator(nn.Module):
    def __init__(self, code_dim=512, n_mlp=8):
        super().__init__()
...
class Discriminator(nn.Module):
    ...
    def forward(self, img, lmark , step=0, alpha=-1):
        for i in range(step, -1, -1):
            index = self.n_layer - i - 1

            if i == step:
                input  = torch.cat((img, lmark), dim=1) 
...
# model = StyledGenerator(512).cuda()
model = Embedder().cuda()
torch.set_printoptions(precision=6)
print (model)
gen_in1, gen_in2 = torch.randn(2, 2, 512, device='cuda').chunk(2, 0)
